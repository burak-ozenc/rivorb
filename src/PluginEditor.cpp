...
RivorbEditor::RivorbEditor(RivorbProcessor &p)
    ...
    postEQLabel.setFont(monoFont(8.5f));
...
void RivorbEditor::resized() {
    ...
    // ── Row 1: Pre-EQ ─────────────────────────────────────────────────────
    int x = 12;
    placeKnob(preHPSlider, preHPLabel, x, row1y);
    x += kw + gap;
    placeKnob(preBell1FreqSlider, preBell1FreqLabel, x, row1y);
    x += kw + gap;
    placeKnob(preBell1GainSlider, preBell1GainLabel, x, row1y);
    x += kw + gap + 8;
    placeKnob(preBell2FreqSlider, preBell2FreqLabel, x, row1y);
    x += kw + gap;
    placeKnob(preBell2GainSlider, preBell2GainLabel, x, row1y);
    x += kw + gap + 8;
    placeKnob(preBell3FreqSlider, preBell3FreqLabel, x, row1y);
    x += kw + gap;
    placeKnob(preBell3GainSlider, preBell3GainLabel, x, row1y);

    // ── Row 1: Saturator ──────────────────────────────────────────────────
    x = 476;
    placeKnob(satDriveSlider, satDriveLabel, x, row1y);
    x += kw + gap;
    placeKnob(satBlendSlider, satBlendLabel, x, row1y);

    // ── Row 2: Reverb ─────────────────────────────────────────────────────
    x = 12;
    placeKnob(roomSizeSlider, roomSizeLabel, x, row2y);
    x += kw + gap;
    placeKnob(dampingSlider, dampingLabel, x, row2y);
    x += kw + gap;
    placeKnob(preDelaySlider, preDelayLabel, x, row2y);
    x += kw + gap;
    placeKnob(wetSlider, wetLabel, x, row2y);

    // ── Row 2: Modulation ─────────────────────────────────────────────────
    x = 294;
    placeKnob(modRateSlider, modRateLabel, x, row2y);
    x += kw + gap;
    placeKnob(modDepthSlider, modDepthLabel, x, row2y);

    // ── Row 2: Compressor ─────────────────────────────────────────────────
    x = 420;
    placeKnob(compThreshSlider, compThreshLabel, x, row2y);
    x += kw + gap;
    placeKnob(compRatioSlider, compRatioLabel, x, row2y);
    x += kw + gap;
    placeKnob(compAttackSlider, compAttackLabel, x, row2y);
    x += kw + gap;
